...
export const getMockTarget = async () => {
  const ModuleDeployment = await deployments.get('MockTarget')
...
export const getAutoSwapExecutor= async () => {
  const validator = await deployments.get('AutoSwapExecutor')
  return await ethers.getContractAt('AutoSwapExecutor', validator.address)
}
...
export const getSafe7579 = async () => {
  const ModuleDeployment = await deployments.get('Safe7579')
